...
const util = {
    ...
    getProjectPath(document) {
        ...
        workspaceFolders.forEach(folder => {
            // 路径替换成linux路径  \\ -> /
            let tmp = '/' + currentFile.split(path.sep).join('/');
            if (tmp.indexOf(folder) === 0) {
                projectPath = folder;
                return;
            }
        })
   ...
   /**
     ...
     */
    readFileDevice: async function (path, callback=null) {
        ...
        rl.on('line', (line) => {
            text_list = line.split(' '); // 按照空格分割字符串
            if (text_list.length > 3 && text_list[2] !==' ' && text_list[2] !== 'Name') {
                deviceArry.push(text_list[2]);
            }
        }).on('close', ()=>{
            console.log('读取完成');
            if (callback !== null) {
     ...
     * @param {*} target : 目标烧录芯片
     */
    machPackFromTarget: function (path, target) {
        ...
        for (let i = target.length; i > 0; --i) {
            let subStr = target.substr(0, i);
            for(let j = 0; j < res.length; ++j) {
                if (res[j].toLowerCase().indexOf(subStr) !== -1) {
                    return res[j];
                }
            }
        }
